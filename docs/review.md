...
- Core vector-calculus identities used in the feature definitions are stated consistently (e.g., divergence $\nabla\cdot u$, advection $(u\cdot\nabla)u$, Laplacian $\nabla^2 u$).
- The spectral differentiation rules described ($\partial/\partial x_j$ via $i k_j$ multiplication; $\partial^2/\partial x_j^2$ via $-k_j^2$) are internally consistent with Fourier differentiation on periodic domains.
...
10.  **The paper claims a “final library comprised 26 distinct candidate terms” but does not provide a complete, explicit, unambiguous list of all 26 analytic terms; in particular, the “additional density-weighted velocity gradient terms derived from $\rho'\nabla u$” are not enumerated, making the library definition mathematically under-specified.**
    
    *Recommendation:* Add a table enumerating all 26 features with explicit formulas and indexing (e.g., $\Theta_1=1$, $\Theta_2=(u\cdot\nabla)u$, …) and clarify exactly which components of $\rho'\nabla u$ are included (e.g., $\rho'\partial u/\partial x$, $\rho'\partial u/\partial y$, …) so the feature matrix $\Theta$ is uniquely defined.

11.  **The status of units/scaling for the ‘kinematic viscosity’ estimate $\nu$ and momentum residual $R_u$ is not mathematically verifiable from the text because it is unclear whether $\nu$ is estimated using raw (physical/nondimensional) fields or standardized fields, and component-wise standardization can change coefficient interpretation unless the transformation is accounted for.**
    
    *Recommendation:* Explicitly state whether derivatives/features and the $\nu$ regression are computed from raw fields or standardized fields; if standardized fields are used, provide the variable transformation and show how $\nu$ (and any inferred coefficients) maps back to the unstandardized equation, or clearly relabel $\nu$ as a coefficient in standardized coordinates.
...
13.  Notation overload: the symbol $u$ is used both for the velocity vector $\mathbf{u}=(u,v,w)$ and for the $x$-component $u$, which makes expressions like $\nabla^2 u$ and $(u\cdot\nabla)u$ potentially ambiguous without context.
...
6.  The term ‘normalized density perturbation’ $\rho'$ is defined as $\rho-1.0$ and later all fields are standardized; the text sometimes refers to $\rho'$ as if it remains only mean-shifted, which can be read as a mild terminology mismatch.
    
    *Recommendation:* Distinguish clearly between ‘density perturbation’ ($\rho-1$) and ‘standardized density perturbation’ (($\rho-1-\mu)/\sigma$) with distinct notation (e.g., $\tilde{\rho}$ for standardized).

7.  The gradient-energy definition $G^2 = \sum_{i,j}(\partial u_i/\partial x_j)^2$ is given without stating whether derivatives are with respect to physical coordinates or grid indices, which affects dimensional interpretation (though not the qualitative use as a selector).
    
    *Recommendation:* Specify whether $x$, $y$, $z$ are physical coordinates (with stated domain length) or grid coordinates, and whether $u_i$ are raw or standardized when computing $G^2$.
...
- • **For periodic domains with nearly incompressible flow, spectral methods for computing spatial derivatives via Fast Fourier Transforms theoretically provide high accuracy for smooth functions, whereas $5$th-order Weighted Essentially Non-Oscillatory (WENO5) finite-difference schemes are specifically designed to robustly handle localized sharp spatial gradients without introducing spurious oscillations.**
  - _Reference(s):_ WENO5

- • **The preliminary physical consistency check is based on the standard incompressible Navier–Stokes momentum equation, in which the temporal derivative of velocity, nonlinear advection term, viscous term proportional to the Laplacian of velocity with kinematic viscosity $\nu$, and the pressure gradient together govern the dynamics of each velocity component in an incompressible fluid.**
...
The paper is primarily methodological and descriptive, with limited formal derivations. The mathematics consists mainly of definitions of transformations ($\rho'=\rho-1$), standardization, standard vector operators used to form a regression feature library (divergence, advection, Laplacian, gradients), and a residual definition $R_u = \partial u/\partial t + (u\cdot\nabla)u - \nu\nabla^2 u$. The main internal-consistency risks are (i) under-specification of the full $26$-term library and (ii) unclear variable scaling/units when interpreting estimated coefficients like $\nu$ after standardization.

### Checked items

1.  ✔ **Density perturbation definition** (Sec. 2.2, p.3 (also reiterated Sec. 3.1, p.6))
    
    - **Claim:** Defines density perturbation as $\rho' = \rho - 1.0$.
    - **Checks:** definition consistency
    - **Verdict:** PASS; confidence: high; impact: minor
    - **Assumptions/inputs:** $\rho$ is already in units/normalization where $1.0$ is the reference (mean) density.
    - **Notes:** Definition is self-consistent and reused consistently.

2.  ✔ **Primary-field standardization** (Sec. 2.2, p.3; Sec. 3.1, p.6)
    
    - **Claim:** Each of $(u,v,w,\rho')$ is standardized by subtracting its global mean and dividing by its global standard deviation to yield mean $0$ and std $1$.
...
6.  ✔ **Divergence definition** (Sec. 2.4, p.4)
    ...
    - **Notes:** Standard definition; consistent with later use of $\nabla\cdot u$.

7.  ✔ **Incompressibility constraint usage** (Sec. 2.4, p.4; Sec. 3.3, p.8-9)
    
    - **Claim:** Uses $\nabla\cdot u = 0$ as a physical constraint to benchmark derivative schemes.
...
8.  ✔ **Advection term component form** (Sec. 2.4, p.4)
    ...
    - **Claim:** States components of $(u\cdot\nabla)u$, e.g., $(u\cdot\nabla)u_x = u\partial u/\partial x + v\partial u/\partial y + w\partial u/\partial z$.
    - **Checks:** vector calculus, notation consistency
    - **Verdict:** PASS; confidence: high; impact: moderate
    - **Assumptions/inputs:** $u$ is velocity vector with components $(u,v,w)$.
    - **Notes:** Correct component expansion for the convective derivative $(u\cdot\nabla)$ applied to a component.
...
10.  ✔ **Gradient energy $G2$ definition** (Sec. 3.2, p.6)
    ...
    - **Claim:** Defines local velocity gradient energy as $G2 = \sum_{i,j} (\partial u_i/\partial x_j)^2$.
    - **Checks:** definition consistency, dimensional consistency
    - **Verdict:** PASS; confidence: medium; impact: minor
    - **Assumptions/inputs:** Indices $i,j$ run over spatial components and coordinates.
    - **Notes:** Definition is internally consistent (squared Frobenius norm of $\nabla u$). Dimensional meaning depends on whether $u$ and $x$ are raw/standardized/physical; this is a clarity gap rather than an algebraic error.

11.  ✔ **Proxy feature: gradient of kinetic-energy-like scalar** (Sec. 2.4-2.5, p.4-5)
    
    - **Claim:** Includes $\nabla(u^2+v^2+w^2)$ as a candidate proxy term.
...
12.  ✔ **Proxy feature: gradient of squared density perturbation** (Sec. 2.4-2.5, p.4-5)
    ...
    - **Claim:** Includes $\nabla(\rho'^2)$ and its component derivatives as candidate terms.
    - **Checks:** algebra, definition consistency
    - **Verdict:** PASS; confidence: high; impact: minor
    - **Assumptions/inputs:** $\rho'$ is a scalar field.
    - **Notes:** Algebraically consistent (gradient of a squared scalar).

13.  ✔ **Proxy feature: gradient of divergence** (Sec. 2.4-2.5, p.4-5)
    
    - **Claim:** Includes $\nabla(\nabla\cdot u)$ and component derivatives as candidate terms.
...
14.  ✔ **Momentum residual definition** (Sec. 2.6, p.5)
    ...
    - **Claim:** Defines $R_u = \partial u/\partial t + (u\cdot\nabla)u - \nu\nabla^2 u$ (and analogously for $v,w$).
    - **Checks:** algebra, notation consistency
    - **Verdict:** PASS; confidence: medium; impact: moderate
    - **Assumptions/inputs:** $\nu$ is scalar coefficient; Laplacian applied to the same component., The symbol $u$ in $(u\cdot\nabla)u$ is the velocity vector; the first $u$ in $\partial u/\partial t$ is the $u$-component.
...
15.  ⚠ **Viscosity regression model (derivation completeness)** (Sec. 2.6, p.5; Sec. 3.4, p.9)
    ...
    - **Claim:** Estimates $\nu$ by linear regression of $\partial u/\partial t$ against $\nabla^2 u$ across domain/time and averages across spatial dimensions.
    - **Checks:** derivation completeness, assumption clarity
    - **Verdict:** UNCERTAIN; confidence: medium; impact: moderate
    - **Assumptions/inputs:** Implicit regression model is $\partial u/\partial t \approx \nu\nabla^2 u$ (plus noise).
...
16.  ⚠ **Feature-library size and completeness (26 terms)** (Sec. 2.5, p.4-5; Sec. 3.5, p.9-10)
    ...
    - **Claim:** States $\Theta$ contains $26$ distinct candidate terms including constants, advection, Laplacians, multiple gradients, cross-terms $u\rho'$, $v\rho'$, $w\rho'$, and additional terms derived from $\rho'\nabla u$.
    ...
    - **Notes:** The analytic definition of $\Theta$ is not fully specified: the paper does not enumerate exactly which components of $\rho'\nabla u$ (a tensor with multiple components) are included, so the claimed total of $26$ terms cannot be verified from the PDF alone.

17.  ✔ **Per-feature standardization of $\Theta$ columns** (Sec. 2.5, p.5; Sec. 3.5, p.10)
    
    - **Claim:** Independently standardizes each column of $\Theta$ to mean $0$ and standard deviation $1$.
...
18.  ⚠ **Interpretation of $\nu$ after standardization** (Sec. 2.2-2.6, p.3-5; Sec. 3.4, p.9)
    
    - **Claim:** Reports $\nu$ as ‘kinematic viscosity’ and uses it in residuals, while earlier stating fields are standardized.
    - **Checks:** dimensional/units consistency, definition consistency
    - **Verdict:** UNCERTAIN; confidence: high; impact: critical
    - **Assumptions/inputs:** Either computations for $\nu$ use raw fields, or there is a mapping from standardized to physical coefficients.
    - **Notes:** Because standardization generally removes physical units and may involve different scalings for $u,v,w$, the coefficient labeled $\nu$ cannot be confirmed as a physical kinematic viscosity without an explicit statement that $\nu$ was computed on raw variables or an explicit back-transformation.
...
- The full analytic specification of the $26$-term feature library $\Theta$ is incomplete in the PDF (not all terms are explicitly listed), preventing a complete internal-consistency check of $\Theta$.
...
1.  ⚠ **C1_domain_points_count** (Methods §2.1 (p.3) and Results §3.5 (p.9): “$128^3$ periodic domain … $10$ time slices” and “evaluated at all $20,971,520$ spatio-temporal points”)
    ...
    - **Verdict:** UNCERTAIN
    - **Notes:** Check was specified ($128^3 \times 10 = 20,971,520$) but not executed due to execution error.

2.  ⚠ **C2_density_perturbation_mean_consistency** (Methods §2.2 (p.3): “$\rho' = \rho - 1.0$” and Results §3.1 (p.5): “$\rho$ exhibited a global mean of $1.0000000000061107$”)
    
    - **Claim:** Given $\rho' = \rho - 1.0$, the mean of $\rho'$ should equal $\operatorname{mean}(\rho) - 1.0$.
    - **Checks:** derived_value_from_definition
    - **Verdict:** UNCERTAIN
    - **Notes:** Only the implied arithmetic could be checked from the stated $\operatorname{mean}(\rho)$; execution did not occur due to execution error.
...
5.  ⚠ **C5_rmse_rhoprime_smooth_vs_sharp_difference** (Results §3.2 (p.6): “RMSE … $0.0250$ for $\rho'$ (smooth) … $0.0242$ (sharp)”)
    
    - **Claim:** Check the absolute and relative difference between the two reported $\rho'$ RMSE values (smooth vs sharp).
...
6.  ⚠ **C6_gradient_energy_ratio_sharp_to_smooth** (Results §3.2 (p.6): “smooth … $G^2 \approx 7.75$” and “sharp … $G^2 \approx 19706.18$”)
    
    - **Claim:** Compute ratio of sharp-region gradient energy to smooth-region gradient energy.
    - **Checks:** ratio_recompute
    - **Verdict:** UNCERTAIN
    - **Notes:** Inputs are approximate ($\approx$) per statement; computation was specified but not executed due to execution error.

7.  ⚠ **C7_derivative_difference_mean_near_zero** (Results §3.3 (p.8): “difference … yielded a mean near zero ($-1.14 \times 10^{-8}$) and … standard deviation ($0.056$)”)
...
9.  ⚠ **C9_incompressibility_max_error_improvement** (Results §3.3 (p.9): max abs error $3.979$ (spectral) vs $4.038$ (WENO5))
    ...
    - **Claim:** Compute absolute and relative improvement in max $|\text{divergence}|$ for spectral vs WENO5.
    - **Checks:** difference_and_ratio_check
    - **Verdict:** UNCERTAIN
    - **Notes:** Deterministic difference/ratio was specified but not executed due to execution error.
